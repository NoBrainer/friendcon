...
$requestResult = $MySQLi_CON->query(
	"SELECT req.status_id, req.num_points
	 FROM points_request req
	 WHERE req.target_uid={$targetUid} AND req.source_uid={$sourceUid} AND req.status_id=0");
if(!$requestResult) die("Accepting request failed [DB-1]");
...
$pointsResult = $MySQLi_CON->query(
	"SELECT u.upoints
	 FROM users u
	 WHERE u.uid={$targetUid}");
if(!$pointsResult) die("Checking points for request failed [DB-2]");
...
$historyQuery =
	"INSERT INTO points_history(from_uid, to_uid, num_points)
	 VALUES ({$targetUid}, {$sourceUid}, {$numPoints})";
$MySQLi_CON->query($historyQuery);

// Send the points
$sendQuery =
	"UPDATE users from_u, users to_u
