...
if ($hasParamAgreeToTerms || $hasParamToggleRegistered || $hasParamTogglePaid || $hasParamTogglePresent || $hasParamSetRegistered || $hasParamSetPaid || $hasParamSetPresent) {
	$userQuery =
		"SELECT u.isPresent, u.isRegistered, u.isPaid, u.agreeToTerms
		 FROM users u
		 WHERE u.uid = {$uid}";
} else {
	die("No changes made");
...
$numRows = 0;
$checkResult = $MySQLi_CON->query(
	"SELECT * FROM registration_stats s
...
if ($isRegistered == 0) {
	...
	$insertQuery =
		"INSERT INTO `registration_stats`(`uid`, `conYear`, `isPaid`, `isPresent`)
