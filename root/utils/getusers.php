...
if(isset($forAdmin) && $isAdmin){
	$userListQuery =
		"SELECT u.uid, u.email, u.name, u.upoints, h.housename, IF(u.isRegistered = -1, 1, 0) AS isHouse
		 FROM users u
	...
	$userListQuery =
		"SELECT u.uid, u.email, u.name, u.isPresent, u.isRegistered, u.isPaid, h.housename
		 FROM users u
		 JOIN house h ON u.houseid = h.houseid
		 ORDER BY u.name ASC, u.isRegistered DESC";
}else if(isset($forTeamSort) && $isAdmin){
	$userListQuery =
		"SELECT u.uid, u.email, u.name, u.isPresent, u.isRegistered, u.isPaid, h.housename, h.houseid
		 FROM users u
		 JOIN house h ON u.houseid = h.houseid
		 WHERE u.isPresent = 1
	...
	$userListQuery =
		"SELECT u.name, u.email, h.housename
		 FROM users u
		 JOIN house h ON u.houseid = h.houseid
...
if(!$userListResult) die("User list query failed [DB-1]");
$userList = array();
...
while($i < $length){
	...
	$attrsPrinted = 0;
	if(isset($uid)){
		if($attrsPrinted > 0) $str = "{$str},"; //add comma
		$str = "{$str}\"uid\":{$uid}";
		$attrsPrinted++;
	}
	if(isset($email)){
