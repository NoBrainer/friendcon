...
if(isset($_GET['housename'])){
	...
	if(!$houseResult) die("Error checking user house [DB-1]");
	...
	if(!$updateResult) die("Failed to set house [DB-2]");
...
$houseQuery =
	"SELECT u.houseid, COUNT(*) AS `count`
	 FROM users u
	 WHERE u.houseid != 0
	 GROUP BY u.houseid";

$houseResult = $MySQLi_CON->query($houseQuery);
if(!$houseResult) die("House query failed [DB-4]");
$houseList = array();
$hasHouse1 = 0;
$hasHouse2 = 0;
$hasHouse3 = 0;
...
while($row = $houseResult->fetch_array()){
	$houseId = $row["houseid"];
	switch($houseId){
		case 1: $hasHouse1=1; break;
		case 2: $hasHouse2=1; break;
		case 3: $hasHouse3=1; break;
		case 4: $hasHouse4=1; break;
	}
...
// Make the update call
$updateResult = $MySQLi_CON->query($updateQuery);
if(!$updateResult) die("Sorting user failed [DB-5]");
